...
line_wavelengths = np.array(
    [3726.0, 3729.0, 4862.0, 4960.0, 5008.0, 6549.0, 6564.0, 6585.0]
)
obs_line_wavelength = np.expand_dims(line_wavelengths, axis=0) * (
    1 + np.expand_dims(redshift, axis=-1)
)
...
gaussian_noise_model = GaussianNoiseModelWavelength(
    sdss_spec_line[line_flux_err_labels].to_numpy().reshape(-1),
    obs_line_wavelength.reshape(-1),
)


def simulation(theta):
    return simulation_SDSS(
        theta, line_wavelengths, interpolated_flux, "random", gaussian_noise_model
    )
