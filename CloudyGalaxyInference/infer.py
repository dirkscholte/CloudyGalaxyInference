...
def fit_model_to_data(sbi_posterior, data_flux, data_flux_error, interpolated_logOH, num_samples=10000, prior_lower_boundary=[0., -1., -4., 0.1, -2.], prior_upper_boundary=[6., 0.7, -1., 0.6, 0.6], plotting=False, plot_name='test', save_samples=False):
    '''
...
def fit_model_to_dataframe(posterior_network, dataframe, identifier_column, line_flux_labels, line_flux_error_labels, output_file, interpolated_logOH, num_samples=10000, prior_lower_boundary=[0., -1., -4., 0.1, -2.], prior_upper_boundary=[6., 0.7, -1., 0.6, 0.6], plotting=False, plot_name='test', save_samples=False):
    '''
    ...
    for i in range(len(dataframe)):
        parameters[i,1:], _ = fit_model_to_data(posterior, dataframe[line_flux_labels].to_numpy()[i], dataframe[line_flux_error_labels].to_numpy()[i], interpolated_logOH, num_samples=num_samples, prior_lower_boundary=prior_lower_boundary, prior_upper_boundary=prior_upper_boundary, plotting=plotting, plot_name=plot_name+'_'+str(i), save_samples=save_samples)
        if i % 100 == 0.0:
