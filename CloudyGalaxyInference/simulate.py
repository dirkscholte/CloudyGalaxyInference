...
def transmission_function_slab(wl, tauV, n=-1.3):
    tau_wl = tauV * (wl / 5500.0) ** n
    return (
        1
        / (2 * tau_wl)
        * (1 + (tau_wl - 1) * np.exp(-tau_wl) - tau_wl**2 * exp1(tau_wl))
    )
...
def simulation_MUSE(
    ...
    dust_geometry="foreground_screen",
):
    """
    ...
    for i in range(len(interpolated_flux)):
        model_line_flux[i] = (
            10 ** theta[0] * interpolated_flux[i](theta[1:-1]) * transmission[i]
        )
    ...
    line_flux, line_flux_error = gaussian_noise_model.add_gaussian_noise(
        model_line_flux,
        (1 + redshift) * line_wavelengths,
        np.ones_like(line_wavelengths) * np.random.uniform(low=0.0, high=100.0),
    )
