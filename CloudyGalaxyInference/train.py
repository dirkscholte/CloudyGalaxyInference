...
def train_MUSE(simulation, save_model_name, save_path='./', num_simulations=1e6, prior_lower_boundary=[0, -1., -4., 0.1, -2.], prior_upper_boundary=[6, 0.7, -1., 0.6, 0.6]):
    '''
    ...
    :return:
    '''
    prior = utils.BoxUniform(low = torch.tensor(prior_lower_boundary),
                         high= torch.tensor(prior_upper_boundary))

    theta, x = simulate_for_sbi(simulation, proposal=prior, num_simulations=num_simulations)
    ...
    for epoch in save_epochs:
        ...
        torch.save(posterior.net, save_path + save_model_name + 'epoch_{}'.format(epoch))
