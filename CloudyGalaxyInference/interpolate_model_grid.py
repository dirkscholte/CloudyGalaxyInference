...
class InterpolateModelGrid:
    def __init__(
        ...
        model_derived_parameter_grid,
        normalize_by="Unnormalized",
    ):
        self.model_line_labels = dict(
            zip(model_line_labels, range(len(model_line_labels)))
        )
        ...
        self.model_parameter_list = [
            np.unique(np.take(model_parameter_grid, index, axis=-1))
            for index in range(model_parameter_grid.shape[-1])
        ]
        ...
        if self.normalize_by == "Unnormalized":
            ...
            self.normalized_flux_grid = self.model_flux_grid / np.expand_dims(
                np.take(self.model_flux_grid, normalization_index, axis=-1), axis=-1
            )
    ...
    def interpolate_flux(self, line_labels):
        ...
        for i in range(len(line_labels)):
            ...
            interpolated_flux.append(
                RegularGridInterpolator(
                    self.model_parameter_list,
                    np.take(self.normalized_flux_grid, index, axis=-1),
                    bounds_error=False,
                    fill_value=None,
                )
            )
    ...
    def interpolate_derived_parameters(self):
        ...
        for i in range(self.model_derived_parameter_grid.shape[-1]):
            interpolated_derived_parameters.append(
                RegularGridInterpolator(
                    self.model_parameter_list,
                    np.take(self.model_derived_parameter_grid, i, axis=-1),
                    bounds_error=False,
                    fill_value=None,
                )
            )
