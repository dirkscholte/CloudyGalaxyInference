...
@u.quantity_input
def galactic_extinction_correction(
    ra: u.degree,
    dec: u.degree,
    obs_wavelengths: u.angstrom,
    spectrum: u.erg * u.cm**-2 * u.s**-1,
):
...
def galactic_extinction_correct_MUSE_df(
    ...
    for galaxy_id in galaxy_ids:
        ...
        column_obs_wls = (
            column_rest_wls * (1 + sub_dataframe["REDSHIFT"].iloc[0]) * u.angstrom
        )
        galactic_correction_factors = galactic_extinction_correction(
            ra,
            dec,
            column_obs_wls,
            np.ones_like(column_rest_wls) * (u.erg * u.cm**-2 * u.s**-1),
        ).value
